...
public:
    ...
    stdio_filebuf();
    stdio_filebuf(int fd,
                  std::ios_base::openmode mode,
                  size_t size = static_cast<size_t>(BUFSIZ));
    stdio_filebuf(FILE* f, std::ios_base::openmode mode,
                  size_t size = static_cast<size_t>(BUFSIZ));
...
template <class CharT, class Traits>
stdio_filebuf<CharT, Traits>::stdio_filebuf(int fd,
                                            std::ios_base::openmode mode,
                                            size_t size)
: extbuf_(0)
, extbufnext_(0)
...
template <class CharT, class Traits>
stdio_filebuf<CharT, Traits>::stdio_filebuf(FILE* f,
                                            std::ios_base::openmode mode,
                                            size_t size)
{
    if (std::has_facet<std::codecvt<char_type, char, state_type> >(this->getloc())) {
