...
void TConstCollection::change(int index, const char * Ident, intptr_t Value)
{
...
void TConstCollection::add(const char * Ident, intptr_t Value, bool IsReadOnly)
{
...
{
   return (((TConstData *)item)->Value == (intptr_t)value);
}
...
int TConstCollection::getIndex(intptr_t Value)
{
   void * i = firstThat(&valueMatch, (void *)&Value);
   if (i) return indexOf(i); else return -1;
}

char * TConstCollection::getId(intptr_t Value)
{
   if (getIndex(Value)!=-1) return ((TConstData*)at(getIndex(Value)))->Id;
     else return 0;
}

intptr_t TConstCollection::getValue(char * Ident)
{
