...
public:
   ...
   ~TConstCollection();
   void add(const char * Ident, intptr_t Value, bool IsReadOnly = false);
   void change(int index, const char * Ident, intptr_t Value);
   int getIndex(char * Ident);
   int getIndex(intptr_t Value);
   char * getId(intptr_t Value);
   intptr_t getValue(char * Ident);
