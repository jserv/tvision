...
{
   for (int EachS=strlen(PathName)-1; EachS>=0; EachS--)
   {
      if ((EachS==0)||(*(PathName+EachS)=='/'))
      {
         strcpy(Ext, "");
...
void skipToNonWord( char *line, unsigned &i )
{
...
char *getWord( char *line, unsigned &i )
{
    unsigned j;
    char *strptr;
...
TTopicDefinition *topicDefinition( char *line, unsigned &i )
{
    unsigned j;
...
TTopicDefinition *topicDefinitionList( char *line, unsigned &i )
{
    unsigned j;
    char w[MAXSTRSIZE];
...
{
    char textName[PATH_MAX];
    char symbName[PATH_MAX];
    fpstream* helpStrm;
